...
import { getSampleOptions } from './utils';
...
const BatchTransfers: React.FC = () => {
  ...
  const handleRowClick = (row: AnyObject) => {
    navigate(`/${PATH_BY_MENU[MENU.TRANSFERS]}/batch/${row.id}/details`, {
      state: {
        batchRowDetails: row,
      },
    });
  ...
  const sampleOptions: any[] = getSampleOptions(preferences);
  const canDownload: boolean = hasPermission(
    restrictionCodes,
  ...
  return (
    <>
      <PageHeader embedKey="TRANSFERS_BATCH">
        {LABEL_BY_SUBMENU[SUBMENU.BATCH]} {LABEL_BY_MENU[MENU.TRANSFERS]}
      </PageHeader>
      <MetaTags
      ...
      <Text className="my-2" color="bodyLight">
        All batch files and transfers are shown here.{' '}
        <a
          href={KNOW_MORE.TRANSFERS.BATCH}
          target="_blank"
...
export default withReadPermission(BatchTransfers, {
  code: 21001,
