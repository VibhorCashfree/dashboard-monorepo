...
beforeEach(() => {
  ...
  jest
    .spyOn(TransfersService, 'createBatch')
    .mockImplementation(() => Promise.resolve({}));

  jest
    .spyOn(TransfersService, 'updateBatch')
    .mockImplementation(() => Promise.resolve({}));

  jest
    .spyOn(TransfersService, 'getErrorLog')
    .mockImplementation(() => Promise.resolve({}));
});
...
describe('In BatchTransfers Container', () => {
  test('checks render', async () => {
    render(<BatchTransfers />, { wrapper: CustomWrapper });
  ...
  test('checks Modals render', async () => {
    ...
    await waitFor(() => {
      expect(
        screen.queryByText('Upload Batch Transfer File'),
      ).toBeInTheDocument();
  ...
  test('checks Modals render', async () => {
    ...
    await waitFor(() => {
      ...
      expect(TransfersService.updateBatch).toHaveBeenCalledWith({
  ...
  test('checks Row click', async () => {
    ...
    expect(mockNavigate).toHaveBeenCalledWith(
      `/${PATH_BY_MENU[MENU.TRANSFERS]}/batch/1234/details`,
      {
        state: {
          batchRowDetails: {
            fileName: 'filename.csv',
