...
// Types
import type { YesBankProps } from '../types';

const YesBank: React.FC<YesBankProps> = ({
  collectedAmount,
  to,
  downtimes,
}) => {
    ...
    <>
      <Paper>
        ...
        <DetailsRow>
          ...
          <div>
            <Text color="bodyLight">Customer ID</Text>
          </div>
          <div>
            <Text className="text-wrap">
              {_get(details, 'connectDetails.customerId', '–')}
            </Text>
          </div>
        </DetailsRow>
...
export default withErrorBoundary(withConnect(YesBank));
