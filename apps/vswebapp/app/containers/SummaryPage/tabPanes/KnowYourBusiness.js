...
const children = (description, docs) => (
  <Space direction="column" gap={1} alignItems="flex-start" s>
...
const KnowYourBusiness = ({ favouriteProducts }) => {
  const navigate = useNavigate();
  const { freeCreditRates } = useContext(AccountContext);
  const products = PRODUCT_MAPPING.kyb;
...
export default withConnect(KnowYourBusiness);
